...
public class AdminBuffetAllProductsController {
    ...
    public String list(Model model,
        ...
        model.addAttribute("products", productRepository.findAllWithCategoryOrdered());
    ...
    public String save(@Valid @ModelAttribute("form") ProductForm form,
        ...
        if (isCreate) {
            audit.log("BUFFET_PRODUCT_CREATE", "product", p.getId(), Map.of("after", after));
        } else {
            audit.log("BUFFET_PRODUCT_UPDATE", "product", p.getId(), Map.of("before", before, "after", after));
        }

        ra.addFlashAttribute("ok", isCreate ? "Товар создан" : "Товар обновлён");
        return "redirect:/admin-buffet/products";
    }
    ...
    public String delete(@RequestParam("id") Long id, RedirectAttributes ra) {
        ...
        try {
            ...
            audit.log("BUFFET_PRODUCT_DELETE", "product", id, Map.of("before", before));
            ra.addFlashAttribute("ok", "Товар удалён");
        } catch (DataIntegrityViolationException e) {
            ra.addFlashAttribute("err", "Нельзя удалить товар: он уже используется в заказах, корзинах, партиях или комбо. Лучше отключите активность товара.");
        }
